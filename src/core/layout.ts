...
export function readmeFile(): RepoFile {
  const text = [
    '# Данные «Трапезы»',
    '',
    'Это хранилище данных приложения «Трапеза» — учёт еды: что съедено, в каком',
    'приёме, блюда и нормы недели. Код приложения лежит в отдельном публичном',
    'репозитории, здесь только записи.',
    '',
    'Репозиторий приватный и должен таким оставаться: внутри то, что и когда',
    'вы ели, и заметки к этому.',
    '',
...
export function parseMeta(text: string): number {
  ...
  if (typeof version !== 'number' || !Number.isInteger(version) || version < 1) {
    throw new Error('В meta.json нет версии схемы. Это не репозиторий «Трапезы»')
  }
  return version
}
