...
export function buildPrompt(specs: readonly ImportSpec[], day: DateStr): string {
  ...
  return [
    ...
    '',
    // Правила про поля — не здесь, а в описаниях разделов: они живут
    // в модуле, а ядро про модули не знает. Свои «Трапезы» — правила 1–3:
    // у «Делу Время» здесь минуты и месяц без числа.
    'Правила:',
    '1. Записей не выдумывай: чего нет в моих данных — не пиши. Необязательное поле, которого нет ' +
    ...
    '2. Даты — ГГГГ-ММ-ДД. Любой вид (24.01.26, 20-02-2026, «3 марта») приводи к нему. Запись, у которой ' +
      'неизвестен день, не пиши, а назови в списке после JSON.',
    '3. Строки и столбцы итогов, проценты и суммы за месяц не переноси: приложение посчитает их само.',
    '4. Разделы, для которых данных нет, не пиши.',
    '5. Ответь одним блоком JSON. Если записей очень много — раздели на несколько блоков, каждый — ' +
      'полный файл в том же формате.',
    '6. После JSON отдельным списком перечисли, что не удалось разобрать или в чём сомневаешься.',
    '',
    'Разделы:',
    '',
    sections.join('\n\n'),
